  ...
  return (
    <footer className="bg-[#1A1A1A] border-t border-gray-800">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {(Array.isArray(header.navLinks) ? header.navLinks : []).map(link => (
                <li key={link.href}><a href={link.href} className="text-[#E8E6DC]/70 hover:text-[#E8E6DC]">{link.text}</a></li>
              ))}
              <li><a href="#contact" className="text-[#E8E6DC]/70 hover:text-[#E8E6DC]">{footerContent.quickLinks.getStarted}</a></li>
              <li><a href="#/admin" className="text-[#8C1E1E] hover:text-[#c58080] font-semibold text-sm mt-2 inline-block">{footerContent.quickLinks.admin}</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {footerContent.services.list.map((service, index) => (
                 <li key={index} className="text-[#E8E6DC]/70">{service}</li>
              ))}
            </ul>
          </div>
          ...
          {contactContent && (
            <div>
              ...
              <ul className="space-y-3">
                <li className="flex items-center gap-2 text-[#E8E6DC]/70">
                  <MailIcon className="h-5 w-5 text-[#8C1E1E]" />
                  {contactContent.details.email.address}
                </li>
                <li className="flex items-center gap-2 text-[#E8E6DC]/70">
                  <PhoneIcon className="h-5 w-5 text-[#8C1E1E]" />
                  {contactContent.details.phone.number}
                </li>
              </ul>
              <div className="mt-4">
                  <h5 className="font-semibold text-[#E8E6DC]">{footerContent.contact.hoursTitle}</h5>
                  {contactContent.details.availability.map((line, index) => (
                      <p key={index} className="text-sm text-[#E8E6DC]/70">{line}</p>
                  ))}
              </div>
            </div>
          )}
      ...
      <div className="bg-black py-4">
          <div className="container mx-auto px-6 text-center text-[#E8E6DC]/50 text-sm">
            {footerContent.copyright}
          </div>
      </div>
       <a href='#contact' className="fixed bottom-6 right-6 bg-[#3C4452] hover:bg-[#2e3540] text-[#E8E6DC] h-12 w-12 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110 z-40" aria-label="Contact">
