...
const ConsultationForm: React.FC<ConsultationFormProps> = ({ sectionId }) => {
    const { content, updateSectionContent } = useContent();
    const sectionData = content.sections.find(s => s.id === sectionId)?.content as ConsultationContent;
    
    const [formData, setFormData] = useState<ConsultationContent | undefined>(sectionData);
    const [status, setStatus] = useState('');
    const [isSaving, setIsSaving] = useState(false);

    useEffect(() => {
        setFormData(content.sections.find(s => s.id === sectionId)?.content as ConsultationContent);
    }, [content, sectionId]);
    ...
    const handleReset = () => {
        if (window.confirm("Are you sure you want to reset this section to its default content?")) {
            const defaultSectionContent = defaultContent.sections.find(s => s.type === 'consultation')?.content;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-800">Edit Consultation Section</h2>
