...
alter table public.intake_submissions add column if not exists age text;
alter table public.intake_submissions add column if not exists fitness_level text;
alter table public.intake_submissions add column if not exists goals text;
alter table public.intake_submissions add column if not exists injuries text;
alter table public.intake_submissions add column if not exists availability text[];
alter table public.intake_submissions add column if not exists preferred_service text;
...
const IntakeSubmissions: React.FC = () => {
    ...
    const fetchSubmissions = useCallback(async () => {
        ...
        if (error) {
            ...
            // Check for table missing (42P01) or column missing/relation errors
            if (error.code === '42P01' || error.message.includes('does not exist') || error.message.includes('Could not find the')) {
                setMissingTable(true);
                setError('The database table is missing or incomplete (missing columns).');
                setShowSetup(true);
            } else {
